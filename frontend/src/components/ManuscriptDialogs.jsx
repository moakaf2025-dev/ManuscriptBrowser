...
export function AboutModal({ onClose }) {
  ...
  const openGuide = async () => {
    setGuideError("");
    try {
      if (!window.msElectron?.openUserGuide) throw new Error("غير متاح في هذه النسخة");
      await window.msElectron.openUserGuide();
    } catch (e) {
      setGuideError(e.message || "تعذّر فتح الدليل");
    }
  };
    ...
    <div className="mr-shortcuts-panel" onClick={onClose} data-testid="mr-about-modal">
      ...
      >
        ...
        <button
          ...
          onClick={openGuide}
          data-testid="mr-about-open-guide"
          style={{ width: "100%", justifyContent: "center", marginBottom: 14 }}
          title="يفتح ملفاً مرفقاً بجانب البرنامج، بمتصفحك المعتاد"
        >
          <Download size={14} /> الدليل المصوّر الكامل لاستخدام البرنامج
        </button>
        {guideError && (
          <div style={{ fontSize: 12, color: "var(--danger, #c85a4a)", marginTop: -8, marginBottom: 14, textAlign: "center" }}>
            {guideError}
          </div>
        )}

        <section style={{ marginBottom: 16 }}>
          <h3 style={{ fontSize: 15, color: "var(--amber)", margin: "0 0 8px" }}>أهمّ ما أُضيف منذ الإصدار الأول</h3>
          <ul style={{ margin: 0, paddingInlineStart: 20, fontSize: 13, lineHeight: 1.9 }}>
            ...
            <li><b>فهرس أرقام الأوراق في PDF المقصوص</b> — تصدير المخطوط مقصوصاً يكتب الآن ١/أ، ١/ب… بنودَ فهرس حقيقية (bookmarks) تظهر في الشريط الجانبي لأي قارئ PDF، لا أرقاماً مطبوعة فقط.</li>
            ...
            <li><b>فتح المخطوط مباشرة من ويندوز</b> — البرنامج مُدرَج الآن في قائمة «فتح باستخدام»؛ انقر ملفاً بزرّ الفأرة الأيمن ليُفتح مباشرة، دون المرور بشاشة البرنامج أولاً.
            </li>
            <li><b>نسخة واحدة من البرنامج</b> — فتح ملف ثانٍ بينما البرنامج يعمل يُسلَّم إلى النافذة المفتوحة بدل تشغيل نسخة جديدة كاملة.</li>
            <li><b>مثبِّت حقيقي لويندوز</b> — إلى جانب النسخة المحمولة، صار للبرنامج مثبِّت يضعه في قائمة ابدأ وسطح المكتب، ولا يمسّ البرنامج الافتراضي لفتح ملفات PDF.</li>
            <li><b>تنبيه قبل فتح مخطوط ضخم</b> — إن تجاوز الملف ٥٠٠ م.ب، يحذّرك البرنامج قبل فتحه بدل أن يتجمّد بصمت.</li>
            <li><b>تذكير بالنسخة الاحتياطية عند الإغلاق</b> — إن كانت هناك حواشٍ أو عناوين أو بطاقات لم تُحفظ في نسخة احتياطية، يسألك البرنامج قبل الإغلاق بدل أن تفقدها دون علم.</li>
            <li><b>تنبيه عند إغلاق مخطوط مقصوص</b> — تقسيم الصفحات المزدوجة عرضٌ مؤقت لا يُحفظ بين الجلسات؛ يذكّرك البرنامج بتصديره أولاً إن أردت الاحتفاظ به.</li>
            <li>وإصلاحات في الذاكرة والأداء: الصفحات المقسومة عند التكبير العالي، ودقّة اللقطة والتعتيم، وتحرير الذاكرة عند إغلاق التبويبات، وزرّ الإغلاق لم يعد يتجمّد إن كان البرنامج مشغولاً.</li>
          </ul>
        </section>
